...
您也可以新建一个`filename.py`的文件，写上Python脚本，然后在命令行中输入`python filename.py`运行。您还可以用IDLE打开这个文件，按F5运行。

您可以在命令提示符中输入`pip install cyaron`，安装 CYaRon 。

如果要更新 CYaRon，您可以输入`pip install --upgrade cyaron`。
...
您可以在终端中输入`pip install cyaron`，安装 CYaRon 。

如果要更新 CYaRon，您可以输入`pip --upgrade`。
