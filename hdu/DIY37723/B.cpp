...
void work()
{
    Matrix<int,MAXN> a,b(true);
    for(int i=1;i<10;i++) a.data[i][i-1]=1;
    for(int i=0;i<10;i++) cin>>a.data[0][i];
    if(k<10)cout<<k%MOD<<endl;
    else
    {
        auto res=Pow(a,b,k-9,MOD);
        int ans=0;
        for(int i=0;i<10;i++) ans+=(res.data[0][i]*(9-i))%MOD;
        cout<<ans%MOD<<endl;
    } 
    return;
}
